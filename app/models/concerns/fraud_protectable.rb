module FraudProtectable
  ...
  def validate_balances_integrity(balance)
    if ENV['FRAUD_PROTECTION'] == 'true' and !balance.authentic?
      ...
      AppLogger.log("balance is unauthentic, balance: #{balance.balance}, real_balance: #{balance.real_balance}, differrence: #{balance.balance.to_i - balance.real_balance.to_i}")
